...
class UsuarioController extends Controller
{
	...
	public function Logear(Request $request)
	{
		...
		$ci = trim($request->input('usuario'));
		$usuario = usuarios::where('ci_usuario', $ci)->first();
		...
		if ($usuario->estado_registro !== 'Aprobado') {
			return response()->json(['success' => false, 'message' => 'El usuario no fue aprobado por el backoffice'], 401);
		}

		if (Hash::check($request->input('password'), $usuario->contraseña)) {
